...
/*   Updated: 2018/01/10 16:35:46 by acourtin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				*fr_draw_burningship(void *arg)
{
	...
	sthreads = (t_thread*)arg;
	m.actual_pix = sthreads->pix_start - 1;
	while (++m.actual_pix < sthreads->pix_end)
	{
		m.pix[0] = m.actual_pix % WIN_WIDTH;
		m.pix[1] = m.actual_pix / WIN_WIDTH;
		m.z[0] = (m.pix[0] + sthreads->smlx->offset_x * sthreads->smlx->zoom \
				/ 400 - (WIN_WIDTH / 2.0)) / sthreads->smlx->zoom;
		m.z[1] = (m.pix[1] + sthreads->smlx->offset_y * sthreads->smlx->zoom \
				/ 400 - (WIN_HEIGHT / 2.0)) / sthreads->smlx->zoom;
		m.tmp[0] = m.z[0];
		m.tmp[1] = m.z[1];
		m.iter = -1;
		...
		color_pix(sthreads->smlx, m.iter, m.actual_pix);
	}
	return (NULL);
}
